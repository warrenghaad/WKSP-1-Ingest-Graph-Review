...
euclid-storyboard-myth-retelling (/storyboard-process)
        ↓ storyboard_v2.json panels with reuse tags + extractable assets
euclid-storyboard-myth-retelling (/storyboard-to-manifest)
        ↓ *_image_manifest.json (IMAGE_NEED blocks, section-centric)
euclid-image-pipeline (/image-pipeline --manifest <manifest>)
...
**Note:** When a storyboard manifest is available, Stage 1 (MANIFEST) uses it directly
instead of re-extracting image needs from lesson text. This ensures the image pipeline
produces exactly the assets the storyboard panels require — no more, no less.
