...
## Integration with euclid-storyboard-myth-retelling

The storyboard tool is an **upstream producer** of image requirements. When storyboard output exists, it REPLACES the lesson-text extraction in Stage 1 (MANIFEST).

**Storyboard-aware workflow:**
```
euclid-gea-research (produces FINDINGS with artifacts)
       ↓
euclid-storyboard-myth-retelling (decomposes myths into 3-Act panels)
       ↓ storyboard_v2.json + asset_reuse_matrix.json
       ↓
euclid-storyboard-myth-retelling /storyboard-to-manifest (bridge)
       ↓ *_image_manifest.json (IMAGE_NEED blocks)
       ↓
euclid-image-pipeline (THIS TOOL — sources, assesses, overlays, recreates)
       ↓ IMAGE_REF manifest
       ↓
Lesson Builder (embeds images into HTML sections)
```

**Stage 1 override:** When `--manifest <path>` is provided with a storyboard-generated manifest:
- SKIP lesson-text extraction — storyboard already defines image needs
- Each IMAGE_NEED carries: section, type, visual_concept, priority, gea_target, extractable_layers
- Priority field determines search urgency (critical → immediate, standard → opportunistic)
- `pipeline_stages` field tells you whether to search museums or generate via AI

**What the storyboard adds that lesson text doesn't:**
- `extractable_assets` per panel — know exactly what layers (background, character, element, fx) to produce
- `reuse_tags` — know which sections each image serves (avoid duplicate sourcing)
- `grade_variants` — narration text per grade level (for text overlays)
- `asset_reuse_matrix.json` — cross-myth view of which panels share assets

**Storyboard tool location:** `../../euclid-storyboard-myth-retelling/`
...
- `../../euclid-storyboard-myth-retelling/references/three-act-process.md` — Storyboard process + panel specs
